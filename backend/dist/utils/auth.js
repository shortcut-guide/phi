 ...
 */
export async function getUserIdFromRequest(req, env) {
    ...
    const valid = await crypto.subtle.verify("HMAC", key, base64urlToBuffer(signatureB64), new TextEncoder().encode(`${headerB64}.${payloadB64}`));
    if (!valid)
        return null;
    try {
 ...
 */
export async function getAndAssertUserId(req, env) {
    const user_id = await getUserIdFromRequest(req, env);
    if (!user_id) {
        throw new Response(JSON.stringify({ error: "unauthorized: user_id required" }), { status: 401, headers: { "Content-Type": "application/json" } });
    }
    return user_id;
}

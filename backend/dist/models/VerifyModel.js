...
export async function updateVerifiedStatus(email) {
...
export async function getVerifiedStatus(userId) {
    const db = getD1UserProfile();
...
export async function insertVerifyToken(tokenId, userId, email) {
    const db = getD1UserProfile();
...
export async function getVerifyToken(tokenId) {
    const db = getD1UserProfile();
    const result = await db.prepare(`
...
export async function deleteVerifyToken(tokenId) {
    const db = getD1UserProfile();
    const result = await db.prepare(`
...
export async function isVerificationExpired(userId, days) {
    const db = getD1UserProfile();
    const result = await db.prepare(`
    SELECT verified_at FROM users WHERE id = ?
  `).bind(userId).first();
    const verifiedAtRaw = result?.verified_at;
    if (!verifiedAtRaw)
        return true;

...
# FTS5 マイグレーションと D1 初期データ投入手順

この節では、ローカル SQLite（FTS5 対応）でのスキーマ作成とデータ投入、ならびに Cloudflare D1 に対する初期スキーマ実行とデータ投入の手順をまとめます。zsh 用コマンドを直接実行できる想定です。

### 事前確認 — FTS5 が利用可能か
ローカルの sqlite3 が FTS5 をサポートしているか確認します。

```sh
# 一時 DB に FTS5 テーブルを作成して確認
echo "CREATE VIRTUAL TABLE test_fts USING fts5(content);" | sqlite3 /tmp/fts_test.db && echo "FTS5 OK" || echo "FTS5 NOT AVAILABLE"

# compile options を確認する場合
sqlite3 /tmp/fts_test.db "PRAGMA compile_options;"
```

- 出力が `FTS5 OK` なら FTS5 利用可。利用不可の場合は後述の代替案を検討してください。

---

### 1) ローカル SQLite にスキーマ適用
プロジェクトの `backend/sql/products.sql`（FTS5 仮想テーブルとトリガーを含む）を利用して DB を作成します。

```sh
mkdir -p backend/data
sqlite3 backend/data/phis.db < backend/sql/products.sql
```

作成後にテーブルを確認:

```sh
sqlite3 backend/data/phis.db "SELECT name, type FROM sqlite_master WHERE type IN ('table','index') ORDER BY type;"
```

---

### 2) ローカルに商品データを投入（推奨: Node スクリプト）
`frontend/src/data/products.json` を `products` テーブルにインポートします。

事前に `better-sqlite3` をインストール（backend ディレクトリで）:
...
スクリプト例: `backend/scripts/import_products.js`

```js
// backend/scripts/import_products.js
const fs = require('fs');
...
```sh
node scripts/import_products.cjs data/phis.db
```

挿入確認:

```sh
sqlite3 data/phis.db "SELECT id, name FROM products LIMIT 5;"
```

注意: `ec_data` は JSON を文字列で保管しています。FTS5 による検索精度を上げるには、あらかじめ `search_terms` のような配列を `ec_data` に含めることが有効です。

---

### 3) Cloudflare D1 にスキーマを適用（Dashboard 推奨）
Cloudflare D1 は環境により FTS5 をサポートしない場合があります。まず Dashboard の Run SQL で以下を試してください。

```sql
CREATE VIRTUAL TABLE test_fts USING fts5(content);
```

成功すれば FTS5 利用可。失敗した場合は代替案を検討してください。（下記参照）

手順（GUI）:
1. Cloudflare Dashboard → Workers → D1 → Create database（名前を付ける）。
2. 作成後、Run SQL に `backend/sql/products.sql` の内容を貼って実行。

CLI（wrangler）が利用できる場合:

```sh
wrangler login
wrangler d1 execute <YOUR_D1_DB_NAME> --file backend/sql/products.sql
```

（wrangler のバージョン差によりコマンドが異なることがあります。Dashboard の Run SQL を推奨）

---

### 4) Cloudflare D1 にデータを投入
大量データを一括投入するとタイムアウトする恐れがあります。以下の方法で安全に投入してください。

推奨: ローカルで `backend/sql/products.sql` → `backend/sql/import_products.sql` を順に `wrangler d1 execute`（または Dashboard API）で実行するスクリプトを用意してください。

生成ワンライナー（リポジトリルートで実行）:

```sh
node -e "const fs=require('fs');const p=require('./frontend/src/data/products.json');const esc=s=>String(s).replace(/'/g,"''");const stmts=p.map(i=>`INSERT OR REPLACE INTO products (id,name,platform,price,ec_data,created_at,updated_at) VALUES ('${esc(i.id)}','${esc(i.name)}','${esc(i.platform||'')}', ${i.price===undefined? 'NULL' : i.price}, '${esc(JSON.stringify(i.ec_data||{}))}', '${esc(i.created_at||new Date().toISOString())}', '${esc(i.updated_at||new Date().toISOString())}');`).join('\n');fs.writeFileSync('backend/sql/import_products.sql',stmts);console.log('wrote backend/sql/import_products.sql');"
```

Dashboard の Run SQL に生成ファイルの一部を貼って（例：500 件ずつ）実行してください。

---

### 5) FTS5 が利用できない場合の代替案
1. D1 が FTS5 非対応なら、D1 側では `search_terms` を設けて `LIKE` 検索で補う（プロジェクト内の worker は既に LIKE フォールバックを行う実装あり）。
2. 外部全文検索サービス（Meilisearch, Algolia, Elasticsearch）を導入して商品データを同期し、高速検索を提供する。
3. 自前で SQLite（FTS5 バイナリ）をホストして運用する（運用コストあり）。

---

### 6) フロントエンド動作確認
1. 開発サーバを起動（プロジェクトの起動手順に従う）。
2. ブラウザで検索画面を開き、キーワードを入力して結果を確認。
3. 期待する API 返却例:

```json
{
  "results": [
    { "id":"B0B2...","name":"...","platform":"amazon","price":1783,"ec_data":{...},"created_at":"...","updated_at":"..." }
  ],
  "suggestions": ["レインコート","犬"]
}
```
...
### 7) 自動化・CI のヒント
- CI で D1 に自動適用する場合は、`backend/sql/products.sql` → `backend/sql/import_products.sql` を順に `wrangler d1 execute`（または Dashboard API）で実行するスクリプトを用意してください。
- 大量データはバッチ分割（例: 500〜1000 件/バッチ）で投入し、各バッチの実行状況をログに残すと復旧が容易です。
